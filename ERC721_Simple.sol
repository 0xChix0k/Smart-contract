...
contract Simple is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  //_maxMint : Mint limit for once transfer > like 2 or 5
  ...
  //Mint NFT
  function NftMint(uint256 _quantity) external payable callerIsUser nonReentrant {
    uint256 tokenId = totalSupply();
    require(tokenId + _quantity <= maxSupply, 'Exceed To MaxSupply.');
    require(_quantity <= maxMint, 'Exceed To MaxMint');
